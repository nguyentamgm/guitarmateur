...
export const SETTINGS: Readonly<Record<string, Setting>> = {
  ...
  sustainLevel: voice('sustainLevel', 0.01, 0.9, 'THE organ dial — lower is percussive, higher carries to the next note'),
  pluckDecaySec: voice('pluckDecaySec', 0.005, 0.3, 'time from peak to sustain'),
  bodyHalfLifeSec: voice('bodyHalfLifeSec', 0.15, 6, 'how long a note takes to halve — raise if notes sound short'),
  ringSec: voice('ringSec', 0.05, 3, 'how far notes ring past their notated length'),
  pickLevel: voice('pickLevel', 0, 1, 'pick attack noise — the main anti-organ cue'),
  slapLevel: voice('slapLevel', 0, 1.5, 'fret slap / flick / scrape — raise to hear techniques'),
  pickBandHz: voice('pickBandHz', 500, 6000, 'attack noise colour (all of them)'),
  legatoLevel: voice('legatoLevel', 0.3, 1.2, 'level of hammered/pulled/slid notes vs picked'),
  bendTimeSec: voice('bendTimeSec', 0.05, 0.6, 'how long a full bend takes to reach pitch'),
  slideTimePerSemitoneSec: voice('slideTimePerSemitoneSec', 0.01, 0.15, 'slide speed, per semitone'),
